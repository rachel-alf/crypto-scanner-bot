...
ratio: 3, // 1:3 risk/reward (change to 2 for 1:2)
maxRiskPercent: 1.27, // Never risk more than 1.5%
minRiskPercent: 0.47, // Never risk less than 0.5%
atrMultiplier: 2.7, // How many ATRs for stop loss
};
...
// // 1. Destructure with default empty arrays to prevent 'undefined' errors
// const { highs = [], lows = [], opens = [], closes = [] } = candles;
// const len = closes.length;

// // Guard against insufficient data
// if (len < 3) return fvgs;

// // 2. Calculate Average Body Size for Displacement Filter
// // We use Math.abs and default to 0 to ensure numbers
// const bodies = opens.map((o, idx) => Math.abs((o || 0) - (closes[idx] || 0)));
// const recentBodies = bodies.slice(-Math.min(len, 50));
// const avgBody =
// recentBodies.reduce((a, b) => a + b, 0) / (recentBodies.length || 1);

// // 3. Loop with safety checks
// for (let i = Math.max(0, len - lookback); i < len - 2; i++) {
// // Use "|| 0" to tell TypeScript: "If this is undefined, use 0"
// const high1 = highs[i] || 0;
// const low1 = lows[i] || 0;

// const open2 = opens[i + 1] || 0;
// const close2 = closes[i + 1] || 0;
// const body2 = Math.abs(open2 - close2);

// const high3 = highs[i + 2] || 0;
// const low3 = lows[i + 2] || 0;

// const currentClose = closes[len - 1] || 0;

// // Safety: Skip if we have no valid price data
// if (high1 === 0 || low3 === 0) continue;

// // DISPLACEMENT CHECK
// const isImpulsive = body2 > avgBody \* 1.1;

// // Bullish FVG
// const bullishGap = low3 - high1;
// if (bullishGap > 0 && isImpulsive && close2 > open2) {
// fvgs.push({
// type: 'BULLISH',
// top: low3,
// bottom: high1,
// index: i + 1,
// filled: currentClose >= low3,
// });
// }

// // Bearish FVG
// const bearishGap = low1 - high3;
// if (bearishGap > 0 && isImpulsive && close2 < open2) {
// fvgs.push({
// type: 'BEARISH',
// top: low1,
// bottom: high3,
// index: i + 1,
// filled: currentClose <= high3,
// });
// }
// }
...
const bodies = opens.map((o, idx) => Math.abs((o || 0) - (closes[idx] || 0)));
const avgBody =
bodies.slice(-50).reduce((a, b) => a + b, 0) / Math.min(50, len);

// Average volume
const avgVolume =
volumes.slice(-20).reduce((a, b) => a + b, 0) / Math.min(20, len);

for (let i = Math.max(0, len - 15); i < len - 2; i++) {
const high1 = highs[i] || 0;
const low1 = lows[i] || 0;
const open2 = opens[i + 1] || 0;
const close2 = closes[i + 1] || 0;
const body2 = Math.abs(open2 - close2);
const volume2 = volumes[i + 1] || 0;
const high3 = highs[i + 2] || 0;
const low3 = lows[i + 2] || 0;
const currentClose = closes[len - 1] || 0;
    ...
    // ✅ QUALITY FILTERS (Multiple moderate filters)

    // 1. Displacement check - moderate
    const isImpulsive = body2 > avgBody * 1.0;
    if (!isImpulsive) continue;

    // 2. Volume confirmation - moderate
    const hasVolume = volume2 > avgVolume * 1.15;
    if (!hasVolume) continue;

    // Bullish FVG
    const bullishGap = low3 - high1;
    const minGapSize = ((high1 + low1) / 2) * 0.0012; // 0.12%

    if (bullishGap > minGapSize && close2 > open2) {
      fvgs.push({
        type: 'BULLISH',
        top: low3,
        bottom: high1,
    ...
    // Bearish FVG
    const bearishGap = low1 - high3;
    if (bearishGap > minGapSize && close2 < open2) {
      fvgs.push({
        type: 'BEARISH',
        top: low1,
        bottom: high3,
...
// Additional factors not in calculateConfidenceFactors
const { relativeStrength } = indicators;

if (relativeStrength) {
if (side === 'LONG' && relativeStrength > 1.05) confidence += 5;
if (side === 'SHORT' && relativeStrength < 0.95) confidence += 5;
}
...
console.log(`⚠️ Conflicting signals: ${conflicts.reason}`, 'warning');
...
if (!candles.closes || candles.closes.length < 50) {
return 'SIDEWAYS';
}
...
const normalizedFutures = normalize(symbol, 'FUTURES');
      ...
      const validSignals = allSignals.filter((signal) => {
        ...
        // ═══════════════════════════════════════════════════════════════
        // ✅ WYCKOFF SIGNAL VALIDATION - Advisory Mode
        // ═══════════════════════════════════════════════════════════════
        ...
        if (SCAN_CONFIG.wyckoffEnabled && wyckoffPhase) {
          ...
          if (wyckoffPhase.confidence > 40) {
            ...
            if (wyckoffTradeSignal.shouldTrade) {
              ...
              if (wyckoffTradeSignal.side !== signal.side) {
                console.log(
                  `   ❌ ${symbol}: Wyckoff wants ${wyckoffTradeSignal.side} but signal is ${signal.side} (conf: ${wyckoffPhase.confidence}%)`
                );
                return false;
              }
              ...
              if (wyckoffPhase.confidence >= 80) {
                signal.confidence = Math.min(95, signal.confidence + 15);
                signal.reason += ` | 🎯 Wyckoff ${wyckoffPhase.phase} (${wyckoffPhase.confidence}%)`;
                console.log(
                  `   ✅ ${symbol}: Wyckoff boost +15% (${wyckoffPhase.phase}, conf: ${wyckoffPhase.confidence}%)`
                );
              } else if (wyckoffPhase.confidence >= 70) {
                signal.confidence = Math.min(95, signal.confidence + 10);
                signal.reason += ` | 📊 Wyckoff ${wyckoffPhase.phase} (${wyckoffPhase.confidence}%)`;
                console.log(
                  `   ✅ ${symbol}: Wyckoff boost +10% (${wyckoffPhase.phase}, conf: ${wyckoffPhase.confidence}%)`
                );
              } else if (wyckoffPhase.confidence >= 50) {
                signal.confidence = Math.min(95, signal.confidence + 5);
                signal.reason += ` | 📈 Wyckoff ${wyckoffPhase.phase} (${wyckoffPhase.confidence}%)`;
                console.log(
                  `   ✅ ${symbol}: Wyckoff boost +5% (${wyckoffPhase.phase}, conf: ${wyckoffPhase.confidence}%)`
                );
              }
            } else {
              // Wyckoff says no trade with meaningful confidence
              console.log(
                `   ⚠️ ${symbol}: Wyckoff advises against trade (${wyckoffTradeSignal.reason}, conf: ${wyckoffPhase.confidence}%)`
              );
              // Reduce confidence instead of blocking
              signal.confidence = Math.max(0, signal.confidence - 15);
              signal.reason += ` | ⚠️ Wyckoff caution`;
            }
          } else {
            // Low Wyckoff confidence - ignore it, don't penalize
            console.log(
              `   ℹ️ ${symbol}: Wyckoff confidence too low (${wyckoffPhase.confidence}%) - not using for signal validation`
            );
          }
        }
...
if (!this.currentWeather) return 'Unknown';
    ...
    const moodMap = {
      ...
      CLAUDE: 'Playing mid-tempo - Trade carefully 🎶',
      SHITTY: 'Playing hardcore - Dangerous! 🔊',
      FUCK_ME: 'TARIFFS MIX! GET OUT! 💥',
    };
