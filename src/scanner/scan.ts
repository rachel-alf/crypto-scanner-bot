...
export class TradingScanner {
  ...
  async scanSymbol(symbol: string): Promise<ExtendedScanResult | null> {
    try {
      ...
      if (!indicators) {
        log(`   ❌ ${symbol}==============>>: Failed to calculate indicators`);
        return null;
      }
      ...
      if (SCAN_CONFIG.wyckoffEnabled) {
        ...
        if (wyckoffPhase.phase === 'MARKDOWN' && wyckoffPhase.confidence > 80) {
          ...
          // return null;
          validSignals.forEach((s) => (s.confidence -= 20));
        }
      ...
      try {
        ...
        if (
          ...
          console.log(`   📊 ${symbol} - 5m trend: ${trend15m}`);
        } else {
          console.log(
            `   ⚠️ ${symbol}: Invalid 5m trend value, defaulting to SIDEWAYS`
          );
        }
  ...
  async scanAll(): Promise<ExtendedScanResult[]> {
    ...
    try {
      ...
      const btcCandles = this.candleManager.getCandles('BTCUSDT', 'FUTURES');
      const marketData = await this.candleManager.getBTCMarketData();
      if (!btcCandles || btcCandles.closes.length < 25) {
        ...
        // console.log(
        //   '🥑 ~ TradingScanner ~ scanAll ~  this.currentWeather:',
        //   this.currentWeather
        // );
    ...
    const symbols = this.candleManager.getInitializedSymbols();
    console.log(`\n📊 Found ${symbols.length} symbols with candles`);
    console.log(
      `🎯 Scanning for signals meeting ${SCAN_CONFIG.minConfidence}% threshold...\n`
    );
  ...
  private getWeatherColor(): string {
    ...
    const colorMap = {
      ...
      STORMY: colors.orange,
      GOLDEN: colors.bgBlue,
    };
...
async function main(): Promise<void> {
  console.log('=== DEBUG: Starting scanner ===');
  console.log('Symbols to scan:', symbols.length, symbols.slice(0, 5));
  ...
  try {
    ...
    console.log(`=== DEBUG: Testing single symbol ${testSymbol} ===`);
    if (!testSymbol) {
      throw new Error('no token');
    }
    const testResult = await scanner.scanSymbol(testSymbol);
    console.log('Test result:', testResult ? 'Found signal' : 'No signal');
    if (testResult) {
      console.log('Signal details:', {
        symbol: testResult.symbol,
        confidence: testResult.confidence,
        strategy: testResult.signal?.strategy,
        side: testResult.signal?.side,
      });
    }
